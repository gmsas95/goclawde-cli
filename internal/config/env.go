...
func LoadEnvFiles() error {
	...
	if home, err := os.UserHomeDir(); err == nil {
		...
		envPaths = append(envPaths,
			// XDG Config directory (most secure for secrets)
			filepath.Join(home, ".config", "myrai", ".env"),
			// XDG Data directory
			filepath.Join(home, ".local", "share", "myrai", ".env"),
			// Legacy location
...
var envAliases = map[string][]string{
	...
	"MYRAI_SKILLS_THREADS_ACCESS_TOKEN":      {"THREADS_ACCESS_TOKEN"},
	"MYRAI_SKILLS_DAUN_ACCESS_TOKEN":         {"DAUN_ACCESS_TOKEN", "DAUN_API_KEY"},
